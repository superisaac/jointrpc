...
func TestMirrorRun(t *testing.T) {
	...
	res, t1, err := c2.CallRPC(ctx1, "add2int", [](interface{}){5, 6}, client.WithTraceId("trace1"))
	assert.Nil(err)
	...
	defer cancel2()
	res3, t2, err := c3.CallRPC(ctx2, "add2int", [](interface{}){15, 16}, client.WithTraceId("test21"))
	assert.Nil(err)
	assert.Equal("test21", t2)
	...
	defer cancel4()
	res4, t3, err := c2.CallRPC(ctx4, "add2int", [](interface{}){15, 16}, client.WithTraceId("trace13"))
	assert.Nil(err)
	assert.Equal("trace13", t3)
